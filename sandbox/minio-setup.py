...
RFI_MASK = np.array(
    [(935.40e6, 960.05e6, 1000.0), (1145.85e6, 1300.90e6, 1000.0), (1519.35e6, 1608.30e6, 1000.0)],
    dtype=[("min_frequency", "f8"), ("max_frequency", "f8"), ("max_baseline", "f8")],
)


BAND_MASK = np.array(
    [(0.0, 0.05), (0.95, 1.0)], dtype=[("min_fraction", "f8"), ("max_fraction", "f8")]
)
...
def put_rfi_mask(client):
    ...
    client.put_object(
        Bucket="models",
        Key="rfi_mask/current.alias",
        ContentType="text/plain",
        Body=b"config/sandbox_v1.alias\n",
    )
    client.put_object(
        Bucket="models",
        Key="rfi_mask/config/sandbox_v1.alias",
        ContentType="text/plain",
        Body=f"../fixed/{filename}\n",
    )
    client.put_object(
        Bucket="models", ContentType="application/x-hdf5", Key=f"rfi_mask/fixed/{filename}", Body=fh
    )
